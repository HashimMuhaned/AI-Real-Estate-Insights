...
export default function PriceTrendTool({}) {
  ...
  const { yoy, trend, trendLabel, trendColor } = useMemo(() => {
    ...
    const thisYearAvg =
      data.slice(4).reduce((s, d) => s + d.price, 0) /
      Math.max(data.length - 4, 1);
      ...
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ left: 10, right: 10 }}>
            ...
            <YAxis
              domain={[
                (dataMin: number) => Math.floor(dataMin / 250000) * 250000,
                (dataMax: number) => Math.ceil(dataMax / 250000) * 250000,
              ]}
              ticks={Array.from({ length: 10 }, (_, i) => i * 250000)}
